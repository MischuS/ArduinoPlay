...
uint32_t Adafruit_VS1053_FilePlayer::stopPlaying(void)
{
  ...
  // wrap it up!
  playingMusic = false;
  uint32_t position = currentTrack.position();
  currentTrack.close();
  return position;
}
...
boolean Adafruit_VS1053_FilePlayer::startPlayingFile(const char *trackname, uint32_t position)
{
  // reset playback
  sciWrite(VS1053_REG_MODE, VS1053_MODE_SM_LINE1 | VS1053_MODE_SM_SDINEW |
                                VS1053_MODE_SM_LAYER12);
  ...
  // We know we have a valid file. Check if .mp3
  // If so, go to specified file position
  if (position != 0)
  {
    currentTrack.seek(position); // jump to given position
  }
  else
  {
    // if .mp3, check for ID3 tag and jump it if present.
    if (isMP3File(trackname))
    {
      currentTrack.seek(mp3_ID3Jumper(currentTrack));
    }
...
void Adafruit_VS1053::startRecordOgg(boolean mic)
{
  ...
  {
    sciWrite(VS1053_REG_MODE, VS1053_MODE_SM_LINE1 | VS1053_MODE_SM_ADPCM | VS1053_MODE_SM_SDINEW);
  }
  sciWrite(VS1053_SCI_AICTRL0, 1024); /* Rec level: 1024 = 1. If 0, use AGC */
  sciWrite(VS1053_SCI_AICTRL1, 1024); /* Maximum AGC level: 1024 = 1. Only used if SCI_AICTRL1 is set to 0. */
  sciWrite(VS1053_SCI_AICTRL2, 0);    /* Miscellaneous bits that also must be set before recording. */
